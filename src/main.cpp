...
//if ((double)COIN/tx.vout[0].nValue == .1 || (double)COIN/tx.vout[0].nValue == .5 || (double)COIN/tx.vout[0].nValue == 1 || (double)COIN/tx.vout[0].nValue == 5 || (double)COIN/tx.vout[0].nValue == 10 || (double)COIN/tx.vout[0].nValue == 50 || (double)COIN/tx.vout[0].nValue == 100 || (double)COIN/tx.vout[0].nValue == 500 || (double)COIN/tx.vout[0].nValue == 1000) //teleport denominations

int32_t is_teleport_denomination(uint64_t value)
{
    uint64_t modval,divval;
    modval = (value % (COIN/1000));
    if ( modval != 0 )
        return(false);
    divval = (value / (COIN/1000));
    while ( divval > 0 )
    {
        modval = (divval % 10);
        if ( modval == 0 )
    ...
    if (!IsCoinBase())
    {
        ...
        if (!IsCoinStake())
        {
            if (nValueIn < GetValueOut()){
                printf("ConnectInputs() : %s value in < value out\n", GetHash().ToString().substr(0,10).c_str());
                return DoS(100, error("ConnectInputs() : %s value in < value out", GetHash().ToString().substr(0,10).c_str()));
            }
            // Tally transaction fees
            int64_t nTxFee = nValueIn - GetValueOut();
            if (nTxFee < 0){
                printf("ConnectInputs() : %s nTxFee < 0\n", GetHash().ToString().substr(0,10).c_str());
           ...
           /* bool isTeleport;
            ...
            nFees += nTxFee;
            if (!MoneyRange(nFees)){
                printf("ConnectInputs() : nFees out of range\n");
...
char *process_jl777_msg(CNode *from,char *msg, int32_t duration)
{
   ...
   fprintf(stderr,"in process_jl777_msg(%s) dur.%d\n",msg,duration);
    if ( SuperNET_retval < 0 )
        return(0);
	if ( msg == 0 || msg[0] == 0 )
	{
