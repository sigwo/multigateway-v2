...
char *makeoffer3_func(char *NXTaddr,char *NXTACCTSECRET,char *previpaddr,char *sender,int32_t valid,cJSON **objs,int32_t numobjs,char *origargstr)
{
    ...
    if ( is_cJSON_Array(objs[5]) != 0 && cJSON_GetArraySize(objs[5]) == 2 )
        baseobj = cJSON_GetArrayItem(objs[5],0), relobj = cJSON_GetArrayItem(objs[5],1);
    else baseobj = relobj = 0;
    if ( sender[0] != 0 && valid > 0 )
        retstr = makeoffer3(NXTaddr,NXTACCTSECRET,flip,srcqty,baseid,relid,baseobj,relobj,quoteid);
...
char *SuperNET_json_commands(struct NXThandler_info *mp,char *previpaddr,cJSON *origargjson,char *sender,int32_t valid,char *origargstr)
{
    ...
    static char *makeoffer3[] = { (char *)makeoffer3_func, "makeoffer3", "V", "baseid", "relid", "quoteid", "srcqty", "flip", "combined", 0 };
