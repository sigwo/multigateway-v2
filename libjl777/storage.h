...
int dbreplace_iQ(int32_t selector,char *keystr,struct InstantDEX_quote *refiQ)
{
    ...
    if ( cursorp != 0 )
    {
        ...
        {
            iQ = data.data;
            printf("%p key.%d: %s, data.size %d: %llu %u %llu | vs %llu %u %llu\n",iQ,n,(char *)key.data,data.size,(long long)iQ->nxt64bits,iQ->timestamp,(long long)iQ->quoteid,(long long)refiQ->nxt64bits,refiQ->timestamp,(long long)refiQ->quoteid);
            if ( iQ->nxt64bits == refiQ->nxt64bits && iQ->quoteid == refiQ->quoteid && refiQ->timestamp > iQ->timestamp )
            {
                clear_pair(&key,&data);
                key.data = keystr;
                key.size = (int32_t)(strlen(keystr) + 1);
