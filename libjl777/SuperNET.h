...
struct combined_amounts { uint64_t frombase,fromrel,tobase,torel; }; struct normal_fields { uint64_t nxt64bits,type; char exchange[9]; };
union quotefields { struct combined_amounts combo; struct normal_fields normal; };
struct InstantDEX_quote { union quotefields U; uint64_t baseamount,relamount;  uint32_t timestamp; uint8_t combined:1,closed:1,sent:1,matched:1,isask:1; char pad[6],gui[5]; };
